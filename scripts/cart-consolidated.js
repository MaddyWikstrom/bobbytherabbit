/**
 * Consolidated Cart System for Bobby Streetwear
 * 
 * This is the primary cart implementation that replaces:
 * - cart.js
 * - cart-checkout-system.js
 * - cart-manager-fix.js
 * 
 * Features:
 * - Reliable image handling with fallbacks
 * - Shopify integration
...
const BobbyCart = {
    ...
    init: function() {
        console.log('🛒 Initializing Bobby Cart System v3.0...');
        ...
        // Set up quick view integration if available
        this.setupQuickViewIntegration();
        
        console.log('✅ Cart system initialized with', this.state.items.length, 'items');
    },
    
    // Set up quick view integration
    setupQuickViewIntegration: function() {
        // Listen for events from Quick View to add products to cart
        document.addEventListener('quickview:addtocart', function(e) {
            if (e.detail && e.detail.product) {
                BobbyCart.addToCart(e.detail.product);
            }
        });
    },
    ...
    addCartStyles: function() {
        ...
        const cartStyles = `
            ...
            .cart-notification-btn.checkout-btn {
                background: linear-gradient(45deg, #a855f7, #3b82f6);
                color: #fff;
            }
            ...
            .cart-count.updated {
                animation: cartCountUpdate 0.6s ease;
            }
            ...
            .cart-icon.pop {
                animation: cartIconPop 0.5s ease;
            }
        ...
        document.addEventListener('click', function(e) {
            ...
            if (addToCartBtn) {
                ...
                if (productContainer) {
                    ...
                    // Visual feedback
                    self.showAddedFeedback(addToCartBtn, product.title, product.image, product.price);
                } else {
    ...
    getEmptyCartHTML: function() {
        return `
            <div class="empty-cart">
                ...
                <button class="continue-shopping-btn" onclick="BobbyCart.closeCart()">
                    Continue Shopping
                </button>
            </div>
        `;
        ...
        notification.innerHTML = `
            ...
            <div class="cart-notification-content">
                ...
                <div class="cart-notification-buttons">
                    <button class="cart-notification-btn view-cart-btn" onclick="BobbyCart.toggleCart()">View Cart</button>
                    <button class="cart-notification-btn checkout-btn" onclick="BobbyCart.proceedToCheckout()">Checkout</button>
                </div>
            </div>
    ...
    proceedToCheckout: function() {
        ...
        // First try to use Shopify integration if available
        if (window.shopifyDebug && typeof window.shopifyDebug.redirectToShopifyCheckout === 'function') {
            try {
                this.showNotification('Redirecting to Shopify checkout...', 'info');
                window.shopifyDebug.redirectToShopifyCheckout();
                return;
            } catch (error) {
                console.error('Error using Shopify checkout:', error);
        ...
        .catch(error => {
            ...
            // Try fallback checkout if available
            if (window.shopifyDebug && typeof window.shopifyDebug.redirectToShopifyCheckout === 'function') {
                setTimeout(() => {
                    this.showNotification('Trying alternative checkout method...', 'info');
                    window.shopifyDebug.redirectToShopifyCheckout();
                }, 1000);
            }
        });
